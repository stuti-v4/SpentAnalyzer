...
def getData(dumpfile):
    ...
    for i in range(len(data_cure_fit)):
        ...
        elements = soup.findAll(
            attrs={"style": "padding-left: 20px; width: 55%"})
    ...
    for i in range(len(data_cure_fit)):
        ...
        attributes_of_sale = {}
        elements_of_total_bill = soup.findAll(
            "td", attrs={"style": "padding-left: 20px; width: 85%"})
        for element in elements_of_total_bill:
            attributes_of_sale[element.text] = element.fetchNextSiblings()[
                0].text
    ...
    for a in total_sales:
        ...
        num_orders.append([cumulative, datetime.datetime.strptime(
            a['date'], "%a, %d %b %Y %H:%M:%S %z").date()])
    total_sales.reverse()
    num_orders = []
    cumulative = 0
    for a in total_sales:
        ...
        cumulative += float(a['Total Payable'])
        num_orders.append([cumulative, datetime.datetime.strptime(
            a['date'], "%a, %d %b %Y %H:%M:%S %z").strftime("%Y%m%d")])
