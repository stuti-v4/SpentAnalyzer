...
def push_report(service):
    ...
    save(repchart, os.path.dirname(os.path.abspath(__file__)) +
         '/data/report.png', scale_factor=1.5)
...
def create_dump(service, company):
    ...
    while 'nextPageToken' in returned_request:
        ...
        returned_request = service.users().messages().list(
...
def push_email(email_content, user_email):
    message = Mail(
        from_email='stutiverma48@gmail.com',
        to_emails=user_email,
        subject='Spent Analyser Report',
        html_content=email_content)
    file_path = './static/images/spentAnalyser.png'
    ...
    attachment.file_name = FileName('spentAnalyser.png')
...
def coerceData(x, y):
    ...
    df = df.reindex(pd.date_range(
        '2019-07-01', periods=12, freq='M')).fillna(0.0)
    # print(df.index)
    # print(df)
    x = df.index.tolist()
    y = df.values
